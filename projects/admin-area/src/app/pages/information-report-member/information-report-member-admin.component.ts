...
export class InformationReportMemberAdminComponent implements OnInit, OnDestroy {
    ...
    getData(startPage: number = this.startPage, maxPage: number = this.maxPage): void {
        ...
        this.getAllPaymentSubs = this.paymentActivityDetailService.getReportPartisipationSuper(this.beginSchedule, this.finishSchedule, startPage, maxPage).subscribe(
            result => {
                console.log(result)
                this.data = result
                this.loading = false
